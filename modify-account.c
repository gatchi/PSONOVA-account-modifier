...
int main()
{
	...
	sprintf (myQuery, "SELECT * FROM account_data WHERE username='%s'", username);
	...
	// Obtain the salt
	sprintf (myQuery, "SELECT regtime FROM account_data WHERE username='%s'", username);
	...
	if (num_rows != 1)
	{
		printf ("Couldn't obtain the salt.\n");
	}
